...
dari Google Sheets ke **Supabase (PostgreSQL)**.
...
| `Piket`     | `"Piket"`          | `Schedule_ID`               | prefix `PKT-`                                |
...
| `uploadPiket(token, data)`           | `kpi_save_piket(p_token, p_data)`               |                                        |
...
### Langkah 6 — Arahkan frontend ke Supabase (10 menit)
...
> - Bila simpan laporan piket error, cek **Console** browser untuk pesan
>   error dari RPC `kpi_save_piket` dan pastikan fungsi RPC tersebut ada di
>   database.

### Langkah 7 — Uji end-to-end (30 menit)
...
| 5   | Unggah laporan piket                       | tersimpan tanpa foto                                      |
...
### Langkah 10 — Setelah migrasi (opsional)
...
"Status", "Poin" numeric, "Status_Kerja", "Catatan_Admin" text
...
`session_ttl_days`, `area_piket`, `supabase_url`.

---

_Dokumen ini dibuat berdasarkan pemeriksaan langsung `code.gs` dan `index.html`
pada repositori ini, sehingga nama tabel, kolom, fungsi, dan aturan bisnisnya
sama dengan yang benar-benar dipakai aplikasi._
