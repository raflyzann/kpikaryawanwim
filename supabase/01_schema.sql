...
--              Bukti_Foto_URL, Poin, Status_Kerja, Catatan_Admin]
-- Bukti_Foto_URL  : tetap dipakai frontend (link publik foto bukti).
-- Bukti_Foto_Path : KOLOM BARU. Path objek di Supabase Storage
--                   (bucket 'piket-bukti'), berguna untuk hapus/rotate file.
create table if not exists public."Piket" (
  ...
  "Status"          text not null default 'Pending'
                    check ("Status" in ('Pending', 'Approved')),
  "Bukti_Foto_URL"  text not null default '#',
...
comment on table public."Piket" is 'Laporan piket + bukti foto (pengganti sheet Piket).';
