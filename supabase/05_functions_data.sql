...
begin
  ...
  select coalesce(jsonb_agg(
           jsonb_build_object(
             ...
             'Bukti_Foto_Path', p."Bukti_Foto_Path",
...
--           "fotoUrl":"https://...","fotoPath":"piket/xxx.jpg"}
-- Foto TIDAK dikirim base64 ke fungsi ini; file diunggah lebih dulu ke
-- Supabase Storage oleh Edge Function `upload-piket` (lihat
-- supabase/functions/upload-piket/index.ts), lalu fungsi ini mencatat
-- barisnya. Dengan begitu payload JSON tetap kecil & cepat.
...
begin
  ...
  v_bucket := coalesce(public._kpi_config('storage_bucket_piket', 'piket-bukti'), 'piket-bukti');

  v_path := nullif(btrim(coalesce(p_data ->> 'fotoPath', p_data ->> 'path', '')), '');
  v_foto_url := nullif(btrim(coalesce(p_data ->> 'fotoUrl', p_data ->> 'url', '')), '');
  if v_foto_url is null and v_path is not null then
    -- URL publik standar Supabase Storage (bucket publik-baca, lihat 02_rls.sql).
    v_foto_url := coalesce(public._kpi_config('supabase_url', ''), '')
                  || '/storage/v1/object/public/' || v_bucket || '/' || v_path;
  end if;
  v_foto_url := coalesce(v_foto_url, '#');

  insert into public."Piket"
    ("Schedule_ID", "User_ID", "Tanggal", "Jenis_Piket", "Status",
     "Bukti_Foto_URL", "Bukti_Foto_Path", "Poin", "Status_Kerja")
  values (
    'PKT-' || gen_random_uuid(),
    v_uid,
